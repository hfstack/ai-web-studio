...
export async function POST(request: NextRequest) {
  try {
    ...
    // 监听进程输出并发送到Socket.IO
    bashProcess.onData((data: string) => {
      const processInfo = processMap.get(port);
      // console.log('Process output received:', data);
      
      if (processInfo) {
        // 总是添加到全局消息队列，确保轮询可以获取到消息
        if (!messageQueue.has(port)) {
          messageQueue.set(port, []);
        }
        messageQueue.get(port)!.push({
          data,
    ...
    bashProcess.onExit(() => {
      ...
      if (processMap.has(port)) {
        ...
        if (processInfo) {
          ...
          // 发送退出消息到队列
          const exitMessage = {
            data: `\r\n\x1b[31mProcess exited at ${new Date().toISOString()}\x1b[0m\r\n`,
            timestamp: new Date().toISOString()
          };
          ...
          if (processInfo.process === bashProcess) {
            ...
            processMap.delete(port);
            messageQueue.delete(port); // 清理消息队列
            // 从数据库中删除已完成的进程
            deleteProcess(port);
          } else {
